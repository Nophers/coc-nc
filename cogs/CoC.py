...
class CoC(commands.Cog):
    ...
    async def searchtagbyseason(self, season, tag):
        cocdb.execute(f'SELECT * FROM `{season}` WHERE tag = "{tag}"')
        result = cocdb.fetchone()
        if result:
            return {'season': season, 'data': result}
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(f'{rawcocdb.FETCHPLAYER_API_URL}{tag.replace("#","%23")}',headers={"Accept": "application/json", "authorization": f"Bearer {rawcocdb.API_KEY}"}) as request:
                    ...
                    playerName = r['name']
            s = time()
            msg = await ctx.send(embed=embeds.Loading._text_to_embed(self.bot, ctx, f'Searching through 20m+ user records for the player **{playerName}**, this could take up to 30s...'))
            async with ctx.typing():
                results =  [await self.searchtagbyseason(season, tag) for season in self.LEGEND_SEASONS]
                e = time()
                        ...
                        if reaction[0].message.id == msg.id:
                            ...
                            if str(reaction[0].emoji) == '✅':
                                for result in results:
                                    ...
                                    embed.add_field(name='Name',value=result["data"][1])
                                    embed.add_field(name='XP',value=result["data"][2])
                                    ...
                                    embed.add_field(name='Clantag',value=result["data"][7])
                                    embed.add_field(name='Clanname',value=result["data"][8])
